...
import {setComboExportVisibility} from './render-results.js';
...
export function initCombosView() {
	...
	const outputHeader = outputTitle.closest('.output-header');

	outputHeader.querySelector('.combos-toolbar')?.remove();

	outputHeader.insertAdjacentHTML(
		'beforeend',
		`<div class="combos-toolbar">
      <button class="btn btn-secondary" id="comboZipSvgBtn" type="button">
        SVG-Export (ZIP)
      </button>
      <button class="btn btn-secondary" id="comboZipPngBtn" type="button">
        PNG-Export (ZIP)
      </button>
      <span class="combos-zip-status" id="comboZipStatus"></span>
    </div>`,
	);
}
...
export function setupSentinelObserver() {
	...
	state.combosObserver = new IntersectionObserver(
		(entries) => {
...
export function loadMoreCombos() {
	if (
		!state.currentEnumSession ||
		state.currentEnumSession.done ||
		state.currentEnumSession.loading
	)
		return;
	...
	state.currentEnumSession.session.pullBatch(
		ENUM_BATCH_SIZE,
		ENUM_BATCH_TIME_SLICE_MS,
		(results, meta) => {
			...
			if (results.length > 0) {
				appendComboCards(
					...
					state.currentGraph,
					results,
					state.currentEnumSession.loadedCount,
					state.currentEnumSession.isTree,
				);
			...
			if (meta.done) {
				...
				if (state.combosObserver) {
					state.combosObserver.disconnect();
					state.combosObserver = null;
				}
				...
				updateCombosStatus(
					state.currentEnumSession.loadedCount,
					meta.stoppedBySafety ? 'safety' : 'done',
				);
